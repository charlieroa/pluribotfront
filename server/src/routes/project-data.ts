...
function getProjectJwt(req: any): { projectId: string; projectUserId: string; email: string; role?: string } | null {
...
router.post('/:id/auth/register', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { type: 'project', projectId: conversationId, projectUserId: user.id, email: user.email, role: user.role },
      JWT_SECRET,
      { expiresIn: '30d' }
    )
...
router.get('/:id/auth/users', async (req, res) => {
  ...
  try {
    ...
    res.json(users.map(u => ({ id: u.id, email: u.email, displayName: u.displayName, role: (u as any).role || 'user', createdAt: u.createdAt })))
  } catch (err) {
    ...
    if (Object.keys(filters).length > 0) {
      ...
      for (const [key, val] of Object.entries(filters)) {
        if (typeof val === 'string') {
          parsed = parsed.filter((row: any) => row[key] !== undefined && String(row[key]).toLowerCase() === val.toLowerCase())
        }
      }
      count = parsed.length
    }
...
router.get('/:id/data/:table/aggregate', async (req, res) => {
  try {
    ...
    const values = parsed.map((r: any) => parseFloat(r[field])).filter(v => !isNaN(v))
    ...
    switch (_op) {
      ...
      default: res.status(400).json({ error: 'Operacion invalida. Usa: sum, avg, min, max' }); return
    }
...
router.get('/:id/data/:table', async (req, res) => {
  try {
    ...
    if (_mine === 'true') {
      const auth = getProjectJwt(req)
      if (auth) {
        parsed = parsed.filter((row: any) => row.user_id === auth.projectUserId)
      }
    }
    ...
    // Pagination
    const offset = _offset ? parseInt(_offset as string, 10) : 0
    const limit = _limit ? Math.min(parseInt(_limit as string, 10), 100) : 100
    ...
    if (typeof _expand === 'string') {
      ...
      for (const exp of expands) {
        ...
        if (!relTable || !TABLE_NAME_RE.test(relTable)) continue
...
router.post('/:id/data/:table', async (req, res) => {
  try {
    ...
    const auth = getProjectJwt(req)
    const data = { ...req.body }
    if (auth) data.user_id = auth.projectUserId
