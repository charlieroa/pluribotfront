...
function detectAspectRatio(message: string): '16:9' | '9:16' | '1:1' {
...
function detectAudience(message: string): string {
  const msg = message.toLowerCase()
  if (msg.includes('restaurante') || msg.includes('cafeter')) return 'Clientes cercanos que toman decisiones visuales y rapidas'
  if (msg.includes('ecommerce') || msg.includes('tienda') || msg.includes('producto')) return 'Compradores listos para comparar y convertir'
  if (msg.includes('saas') || msg.includes('software') || msg.includes('app')) return 'Usuarios que necesitan entender valor rapido'
  return 'Audiencia fria que necesita entender el beneficio en segundos'
}

function detectGoal(message: string): string {
  const msg = message.toLowerCase()
  if (msg.includes('venta') || msg.includes('vende') || msg.includes('compr')) return 'conversion'
  if (msg.includes('anuncio') || msg.includes('ads') || msg.includes('campana')) return 'performance'
  if (msg.includes('marca') || msg.includes('branding')) return 'brand_awareness'
  return 'engagement'
}

function detectVisualStyle(message: string): string {
  const msg = message.toLowerCase()
  if (msg.includes('premium') || msg.includes('lujo')) return 'Premium cinematic'
  if (msg.includes('minimal') || msg.includes('clean')) return 'Minimal modern'
  if (msg.includes('ugc') || msg.includes('testimonial')) return 'UGC social proof'
  if (msg.includes('energet') || msg.includes('vibrante')) return 'Bold high energy'
  return 'Commercial social ad'
}

function detectPlatform(aspectRatio: '16:9' | '9:16' | '1:1', message: string): string {
  const msg = message.toLowerCase()
  ...
  if (aspectRatio === '9:16') return 'Instagram Reels'
...
function buildScenes(subject: string, message: string): WorkflowSceneDraft[] {
  const style = detectVisualStyle(message)
  const goal = detectGoal(message)
  const durations: Array<'3' | '5' | '10'> = goal === 'performance' ? ['3', '3', '5'] : ['5', '5', '5']
...
function buildWorkflowDraft(message: string) {
...
router.post('/sequence-video', authMiddleware, async (req, res) => {
  try {
    const { scenes } = req.body as { scenes?: Array<{ videoUrl?: string | null }> }
