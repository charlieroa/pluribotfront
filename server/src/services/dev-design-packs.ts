type ProjectMode = 'landing' | 'ecommerce' | 'dashboard' | 'crm' | 'system' | 'generic'

function inferProjectMode(task: string): ProjectMode {
  const lower = task.toLowerCase()
  if (/(landing|sitio web|pagina web|marketing|saas)/.test(lower)) return 'landing'
  if (/(ecommerce|tienda|catalogo|checkout|carrito|zapatos|ropa|moda|sneakers)/.test(lower)) return 'ecommerce'
  if (/(crm|pipeline|clientes|leads|ventas)/.test(lower)) return 'crm'
  if (/(dashboard|metricas|analytics|reportes|kpi|panel)/.test(lower)) return 'dashboard'
  if (/(sistema|inventario|reservas|gestion|hospital|barberia|escuela|admin)/.test(lower)) return 'system'
  return 'generic'
}
...
function getModeComposition(mode: ProjectMode): string[] {
  switch (mode) {
    case 'landing':
      return [
...
export function buildDevDesignPackContext(task: string): string {
