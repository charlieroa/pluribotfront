 ...
 */
export function isCodeProject(content: string): boolean {
 ...
 */
export function buildCodeProjectHtml(content: string): string {
  const files: FileEntry[] = JSON.parse(content)
  ...
  const isConfig = (p: string) => /vite\.config|tailwind\.config|postcss\.config/.test(p)
  const isMain = (p: string) => /^(src\/)?(main|index)\.(jsx?|tsx?)$/.test(p)

  const sourceFiles = files.filter(f =>
    /\.(jsx?|tsx?)$/.test(f.path) && !isConfig(f.path) && !isMain(f.path)
  )

  // Topological sort: resolve local imports by matching filenames
  // Build lookup: basename (without ext) → file entry
  const fileByBasename: Record<string, FileEntry> = {}
  for (const f of sourceFiles) {
    ...
    if (!fileByBasename[shortName]) fileByBasename[shortName] = f
  }

  // For each file, find local imports (./X or ../X patterns)
  const deps = new Map<FileEntry, Set<FileEntry>>()
  for (const f of sourceFiles) {
    const fileDeps = new Set<FileEntry>()
  ...
  const inDegree = new Map<FileEntry, number>()
  for (const f of sourceFiles) inDegree.set(f, 0)
  ...
  const globalReactImports = new Set<string>()
  // For external deps: track { originalName → Set<alias> } (e.g., X as Y means original=X, alias=Y)
  const globalDepImports: Record<string, { named: Set<string>; aliases: Map<string, string>; defaults: Set<string> }> = {}
  ...
  function collectImports(code: string) {
    for (const line of code.split('\n')) {
      ...
      if (namedMatch) {
        ...
        if (mod === 'react-dom' || mod === 'react-dom/client') continue
        if (mod.startsWith('.') || mod.startsWith('/')) continue
        if (!globalDepImports[mod]) globalDepImports[mod] = { named: new Set(), aliases: new Map(), defaults: new Set() }
      ...
      if (defaultMatch) {
        ...
        if (mod === 'react' || mod === 'react-dom' || mod === 'react-dom/client') continue
        if (mod.startsWith('.') || mod.startsWith('/')) continue
        if (!globalDepImports[mod]) globalDepImports[mod] = { named: new Set(), aliases: new Map(), defaults: new Set() }
  ...
  for (const f of sourceFiles) collectImports(f.content)
