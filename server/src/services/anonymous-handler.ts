...
export async function handleAnonymousMessage(conversationId: string, userText: string): Promise<void> {
  ...
  if (lowerText.includes('precio') || lowerText.includes('costo') || lowerText.includes('plan') || lowerText.includes('gratis')) {
    ...
    responseText = `Para diseno tenemos a **Pixel**, nuestro disenador visual. Crea logos, branding, posts para redes, banners, flyers, stories y moodboards con Ideogram, Unsplash y remocion de fondo.\n\nPara activarlo, solo necesitas registrarte. Es gratis!`
