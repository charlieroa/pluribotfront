...
export async function handleAnonymousMessage(conversationId: string, userText: string): Promise<void> {
  ...
  if (lowerText.includes('precio') || lowerText.includes('costo') || lowerText.includes('plan') || lowerText.includes('gratis')) {
    ...
    responseText = `Para diseno tenemos a **Pixel**, nuestro disenador visual. Crea logos, branding, posts para redes, banners, flyers, stories, moodboards y todo lo grafico con Midjourney.\n\nPara sitios web funcionales (landings, apps, dashboards), tenemos a **Logic**, nuestro full-stack web builder.\n\nPara activarlos, solo necesitas registrarte. Es gratis!`
