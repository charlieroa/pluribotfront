...
import { orchestratorConfig, REFINE_AGENT_IDS } from '../config/agents.js';
...
router.post('/refine-step', optionalAuth, async (req, res) => {
    const { conversationId, text, instanceId } = req.body;
    const userId = req.auth?.userId ?? 'anonymous';
    try {
        const plan = getExecutingPlan(conversationId);
        if (!plan) {
...
async function processMessage(conversationId, text, userId, modelOverride, imageUrl) {
    ...
    await provider.stream(orchestratorConfig.systemPrompt, messages, {
        ...
        onComplete: (fullText, usage) => {
            ...
            try {
                ...
                orchestratorOutput = JSON.parse(cleanText);
            }
            catch {
                orchestratorOutput = {
