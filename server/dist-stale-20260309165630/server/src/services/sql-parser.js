...
function parseCreateTable(sql) {
    ...
    for (const part of parts) {
        ...
        if (/^(PRIMARY|UNIQUE|FOREIGN|CHECK|CONSTRAINT)\s/i.test(trimmed))
            continue;
        ...
        if (colName === 'id' && /UUID|SERIAL|INTEGER.*PRIMARY/i.test(trimmed))
            continue;
...
export function extractAndParseSql(html) {
    const tables = [];
...
export function schemaToContext(tables) {
    if (tables.length === 0)
        return '';
