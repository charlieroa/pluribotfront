...
export class AnthropicProvider {
    ...
    temperature;
    budgetTokens;
    constructor(apiKey, model, maxTokens = 32768, temperature, budgetTokens = 0) {
    ...
    buildRequestParams(systemPrompt) {
        const params = {
            model: this.model,
            ...
            const mappedMessages = messages.map((m, i) => {
                ...
                const content = m.images && m.images.length > 0
                    ? [
                        ...
                        { type: 'text', text: m.content, ...(shouldCache ? { cache_control: { type: 'ephemeral' } } : {}) },
                    ]
                    : shouldCache
