...
import { setExecutingPlan } from './plan-cache.js';
...
export async function refineStep(plan, step, userFeedback, userId) {
    ...
    const messages = [
        { role: 'user', content: `${step.task}${contextBlock}` },
    ...
    if (isDevAgent) {
        try {
            ...
            broadcast(conversationId, { type: 'error', message: `Error al refinar proyecto: ${err.message}` });
            return;
        }
    }
    else {
