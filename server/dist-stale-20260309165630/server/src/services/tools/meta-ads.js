...
async function metaFetch(url, options) {
    ...
    if (!resp.ok) {
        const error = data.error?.message || resp.statusText;
...
export const metaAdsTools = [
    {
        ...
        execute: async (input, context) => {
            ...
            const data = await metaFetch(`${GRAPH_API}/${adAccountId}/campaigns?fields=id,name,status,objective,daily_budget,lifetime_budget,start_time,stop_time,insights.date_preset(last_7d){spend,impressions,clicks,ctr,cpc}&limit=${limit}&access_token=${token}`);
            if (!data.data?.length) {
    ...
    },
    {
        ...
        execute: async (input, context) => {
            const { token, adAccountId } = await getMetaCredentials(context.userId);
            const genderMap = { all: [0], male: [1], female: [2] };
            const genderValue = genderMap[input.genders || 'all'] || [0];
            const countries = (input.countries || 'MX').split(',').map(c => c.trim());
            const targeting = {
            ...
            if (input.interests) {
                const interestIds = input.interests.split(',').map(id => ({ id: id.trim() }));
            ...
            return `Ad set creado exitosamente.\nID: ${data.id}\nNombre: ${input.name}\nCampaña: ${input.campaignId}\nTargeting: ${countries.join(', ')}, ${input.ageMin || 18}-${input.ageMax || 65} años`;
        },
    },
    {
        name: 'meta_create_ad',
        ...
        execute: async (input, context) => {
            ...
            const creativeParams = new URLSearchParams({
                ...Object.fromEntries(Object.entries(creativeData).map(([k, v]) => [k, typeof v === 'string' ? v : JSON.stringify(v)])),
                access_token: token,
            });
