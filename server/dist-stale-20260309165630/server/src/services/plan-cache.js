...
export async function setPendingPlan(messageId, plan) {
...
export async function getPendingPlan(messageId) {
...
export async function removePendingPlan(messageId) {
    await prisma.pendingPlan.delete({ where: { id: messageId } }).catch(() => { });
}
// Executing plan management (parallel groups)
export async function setExecutingPlan(conversationId, plan) {
...
export async function getExecutingPlan(conversationId) {
    const row = await prisma.executingPlan.findUnique({ where: { conversationId } });
    if (!row)
        return undefined;
    return {
        steps: JSON.parse(row.stepsJson),
        executionGroups: JSON.parse(row.executionGroupsJson),
        currentGroupIndex: row.currentGroupIndex,
        completedInstances: JSON.parse(row.completedInstances),
        agentOutputs: JSON.parse(row.agentOutputs),
...
export async function removeExecutingPlan(conversationId) {
