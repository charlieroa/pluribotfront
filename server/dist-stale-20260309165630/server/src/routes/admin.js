...
import { getBudgets, setBudget, clearBudget, calculateRemaining } from '../config/provider-budgets.js';
const router = Router();
...
router.post('/budgets', superadminAuth, async (req, res) => {
    try {
        const { provider, budget } = req.body;
        ...
        if (provider === 'midjourney') {
            ...
            currentCost = toolEntries.filter(e => e.description.includes('generate_image')).length * 0.05;
        }
        else {
            for (const record of usageByModel) {
...
router.patch('/users/:id', superadminAuth, async (req, res) => {
    try {
        const userId = req.params.id;
        const { role, planId, creditBalance } = req.body;
        const data = {};
        if (role && ['user', 'agent', 'org_admin', 'superadmin'].includes(role))
            data.role = role;
        if (planId && plans[planId]) {
            data.planId = planId;
            await resetCreditsForPlan(userId, planId);
