...
router.get('/:id', optionalAuth, async (req, res) => {
    ...
    const instanceIds = [...new Set(conversation.deliverables
            .map((d) => d.instanceId)
            .filter(Boolean))];
    const versionCountMap = {};
    for (const iid of instanceIds) {
        versionCountMap[iid] = conversation.deliverables.filter((d) => d.instanceId === iid).length;
    }
...
router.get('/:convId/deliverables/:deliverableId/versions', optionalAuth, async (req, res) => {
    const { convId, deliverableId } = req.params;
    try {
