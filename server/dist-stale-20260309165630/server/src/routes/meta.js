...
router.get('/callback', async (req, res) => {
    try {
        ...
        const tokenResp = await fetch(`${GRAPH_API}/oauth/access_token?client_id=${appId}&redirect_uri=${encodeURIComponent(redirectUri)}&client_secret=${appSecret}&code=${code}`);
        if (!tokenResp.ok) {
        ...
        const longTokenResp = await fetch(`${GRAPH_API}/oauth/access_token?grant_type=fb_exchange_token&client_id=${appId}&client_secret=${appSecret}&fb_exchange_token=${tokenData.access_token}`);
...
router.get('/accounts', authMiddleware, async (req, res) => {
    try {
        ...
        const resp = await fetch(`${GRAPH_API}/me/adaccounts?fields=id,name,account_status,currency&access_token=${user.metaAccessToken}`);
        if (!resp.ok) {
