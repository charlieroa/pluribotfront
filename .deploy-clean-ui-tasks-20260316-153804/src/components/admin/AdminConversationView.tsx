  ...
  const handleResumeAI = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/conversations/${conversationId}/message`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ text: 'Continuar con IA' }),
      })
      if (res.ok) {
        fetchConversation()
      }
    } catch (err) {
      console.error('[Admin] Resume AI error:', err)
    } finally {
      setSending(false)
    }
  }
        ...
        <div className="flex items-center gap-2">
          {conversation.needsHumanReview && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-violet-600 bg-violet-500/10 hover:bg-violet-500/20 rounded-lg transition-all"
            >
              <UserCircle size={14} />
              Devolver a IA
            </button>
