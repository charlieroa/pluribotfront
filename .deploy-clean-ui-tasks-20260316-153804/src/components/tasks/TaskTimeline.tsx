...
import { ChevronLeft, ChevronRight, Calendar, ExternalLink, CheckCircle2, ListTodo, Activity, PackageCheck } from 'lucide-react'
...
const columns: { id: KanbanTask['status']; label: string; dotColor: string; headerBg: string }[] = [
  { id: 'todo', label: 'Planeado', dotColor: 'bg-slate-400', headerBg: 'bg-slate-500/10' },
  { id: 'doing', label: 'Ejecutando', dotColor: 'bg-amber-400', headerBg: 'bg-amber-500/10' },
  { id: 'done', label: 'Entregado', dotColor: 'bg-emerald-500', headerBg: 'bg-emerald-500/10' },
]
...
const TaskTimeline = ({ tasks, agents, onTaskClick, onFinalizeTask }: TaskTimelineProps) => {
  ...
  const plannedCount = tasks.filter(task => task.status === 'todo').length
  const executingCount = tasks.filter(task => task.status === 'doing').length
  const deliveredCount = tasks.filter(task => task.status === 'done').length

  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-page">
      <div className="px-4 md:px-6 pt-4 md:pt-5 pb-3 border-b border-edge bg-surface">
        <div className="grid gap-2 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-500/15 bg-slate-500/5 px-4 py-3">
            <div className="flex items-center gap-2 text-slate-600">
              <ListTodo size={14} />
              <p className="text-[11px] font-bold uppercase tracking-wide">Planeado</p>
            </div>
            <p className="mt-2 text-2xl font-bold text-ink">{plannedCount}</p>
            <p className="text-[11px] text-ink-faint mt-1">Tareas que ya quedaron definidas.</p>
          </div>
          <div className="rounded-2xl border border-amber-500/15 bg-amber-500/5 px-4 py-3">
            <div className="flex items-center gap-2 text-amber-600">
              <Activity size={14} />
              <p className="text-[11px] font-bold uppercase tracking-wide">Ejecutando</p>
            </div>
            <p className="mt-2 text-2xl font-bold text-ink">{executingCount}</p>
            <p className="text-[11px] text-ink-faint mt-1">Agentes trabajando ahora mismo.</p>
          </div>
          <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/5 px-4 py-3">
            <div className="flex items-center gap-2 text-emerald-600">
              <PackageCheck size={14} />
              <p className="text-[11px] font-bold uppercase tracking-wide">Entregado</p>
            </div>
            <p className="mt-2 text-2xl font-bold text-ink">{deliveredCount}</p>
            <p className="text-[11px] text-ink-faint mt-1">Recursos listos para abrir o revisar.</p>
          </div>
        </div>
      </div>
