...
import { Check, X, FileText, ExternalLink, KanbanSquare, Play, ChevronRight, Pencil, Layers, UserCircle, FolderPlus, Star, Crown, CheckCircle2, Globe, Palette, Video, Code2, FileCode } from 'lucide-react'
          ...
          <div>
                ...
                <div key={i} className="relative">
                  <ImageWithSkeleton
                    ...
                    onClick={() => {
                      ...
                      ;(window as any).__selectedImageForRefine = { src: url, index: i, deliverableId: m.attachment?.deliverable?.id }
                      if (m.attachment?.deliverable) {
                        onOpenDeliverable?.(m.attachment.deliverable, url)
                      }
                    }}
                  />
                  {selectedImageSrc === url && (
                    <div className="absolute inset-2 pointer-events-none rounded-lg border-2 border-emerald-500 shadow-[0_0_0_1px_rgba(16,185,129,0.25)]">
                      <div className="absolute top-2 right-2 flex items-center gap-1 rounded-full bg-emerald-500 px-2 py-1 text-[10px] font-bold text-white">
            ...
            <p className="text-xs text-ink-faint mt-2">
              {m.attachment.images.length > 1
                ? `Haz clic en la imagen que quieres usar y luego pide el ajuste en el chat.`
                : `Haz clic en la imagen para seleccionarla y luego pide cambios en el chat.`}
            </p>
          </div>
        )}
