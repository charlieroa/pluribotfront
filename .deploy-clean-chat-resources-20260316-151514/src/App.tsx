...
const App = () => {
  ...
  const [sidePanelTab, setSidePanelTab] = useState<'chat' | 'results' | 'resources' | 'edit' | 'settings'>('chat')
                ...
                {chatPanelVisible && (
                  ...
                  `}>
                    <div className="flex border-b border-edge flex-shrink-0">
                      ...
                      <button
                        onClick={() => setSidePanelTab('results')}
                        className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-[11px] font-medium transition-colors ${
                          sidePanelTab === 'results'
                            ? 'text-ink border-b-2 border-emerald-500 bg-surface'
                            : 'text-ink-faint hover:text-ink bg-subtle/30'
                        }`}
                      >
                        <Package size={13} /> Resultados
                        {chat.kanbanTasks.filter(t => t.status === 'done' && t.deliverable).length > 0 && (
                          <span className="text-[9px] font-bold bg-emerald-500/15 text-emerald-600 px-1 py-0.5 rounded-full">
                            {chat.kanbanTasks.filter(t => t.status === 'done' && t.deliverable).length}
                          </span>
                        )}
                      </button>
                      <button
                      ...
                      >
                        ...
                        {conversationResources.length > 0 && (
                          <span className="text-[9px] font-bold bg-violet-500/15 text-violet-600 px-1 py-0.5 rounded-full">
                            {conversationResources.reduce((total, group) => total + group.items.length, 0)}
                          </span>
                        )}
                      </button>
                    ...
                    {sidePanelTab === 'chat' || isImageOnlyDeliverable ? (
                      ...
                      <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-4">
                        {(() => {
                          const doneTasks = chat.kanbanTasks.filter(t => t.status === 'done' && t.deliverable)
                          const doingTasks = chat.kanbanTasks.filter(t => t.status === 'doing' || t.status === 'todo')

                          const groupByType = (tasks: typeof doneTasks) => {
                            const groups: Record<string, typeof doneTasks> = {}
                            for (const t of tasks) {
                              const label = t.deliverable?.botType === 'dev' ? 'Apps' :
                                t.deliverable?.botType === 'web' ? 'Webs' :
                                t.deliverable?.botType === 'video' ? 'Videos' :
                                t.deliverable?.botType === 'brand' ? 'Logos' :
                                t.deliverable?.type === 'copy' ? 'Contenido' : 'Otros'
                          ...
                          if (doneTasks.length === 0 && doingTasks.length === 0) {
                            return (
                              <div className="text-center py-12">
                                <Package size={32} className="mx-auto text-ink-faint/20 mb-3" />
                                <p className="text-xs text-ink-faint">Aun no hay resultados en esta conversacion</p>
                              </div>
                            )
                          }

                          const grouped = groupByType(doneTasks)

                          return (
                            <>
                              {doingTasks.length > 0 && (
                                <div>
                                  <p className="text-[10px] font-bold text-ink-faint uppercase tracking-wider mb-2">En proceso</p>
                                  <div className="space-y-1.5">
                                    {doingTasks.map(t => (
                                      <div key={t.id} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg border border-amber-500/20 bg-amber-500/5">
                                        <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse flex-shrink-0" />
                                        <span className="text-xs font-medium text-ink truncate flex-1">{t.title}</span>
                                        <span className="text-[10px] text-ink-faint">{t.agent}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                              ...
                              {Object.entries(grouped).map(([label, items]) => (
                                <div key={label}>
                                  <p className="text-[10px] font-bold text-ink-faint uppercase tracking-wider mb-2">{label}</p>
                                  <div className="space-y-1.5">
                                    {items.map(t => (
                                      <button
                                        key={t.id}
                                        onClick={() => {
                                          if (t.deliverable) {
                                            setActiveDeliverable(t.deliverable)
                                            setSidebarCollapsed(true)
                                            setChatPanelVisible(true)
                                          }
                                        }}
                                      ...
                                      >
                                        <div className="w-6 h-6 rounded-md flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0"
                                          style={{ backgroundColor: t.deliverable?.botType === 'dev' ? '#f59e0b' : t.deliverable?.botType === 'web' ? '#a855f7' : t.deliverable?.botType === 'video' ? '#ef4444' : '#6b7280' }}
                                        >
                                          {t.deliverable?.botType?.charAt(0)?.toUpperCase() || '?'}
                                        </div>
                                        <div className="min-w-0 flex-1">
                                          <p className="text-xs font-semibold text-ink truncate">{t.title}</p>
                                          <p className="text-[10px] text-ink-faint truncate">{t.agent}</p>
                                        </div>
                                        <span className="text-[10px] font-semibold text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded-full flex-shrink-0">Listo</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
