...
export default function Modal({ open, onClose, title, children, className = '' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center" onClick={onClose}>
      <div className={\`bg-white dark:bg-slate-900 text-foreground border border-border rounded-2xl shadow-float w-full max-w-md mx-4 animate-scale-in \${className}\`} onClick={e => e.stopPropagation()}>
        {title && (
...
export default function DropdownMenu({ trigger, items }: DropdownMenuProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-48 border border-border rounded-xl shadow-float z-50 py-1 animate-scale-in backdrop-blur-xl bg-white dark:bg-slate-900">
          {items.map((item, i) => (
            <button
              key={i}
...
export default function Tooltip({ content, children }: TooltipProps) {
  ...
  return (
    <div className="relative inline-block" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      {children}
      {show && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1.5 bg-white dark:bg-slate-900 text-foreground border border-border text-xs font-medium rounded-lg whitespace-nowrap shadow-float animate-fade-in z-50">
          {content}
