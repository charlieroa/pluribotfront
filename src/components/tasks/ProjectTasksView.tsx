...
interface ProjectTasksViewProps {
  ...
  currentConversationTasks?: KanbanTask[]
  agents: Agent[]
  onBackToChat: () => void
...
const API_BASE = '/api'
const CURRENT_WORKSPACE_ID = '__current_workspace__'
...
const ProjectTasksView = ({
  ...
  const hasCurrentWorkspaceTasks = currentConversationTasks.length > 0
  const fallbackProjects: ProjectItem[] = hasCurrentWorkspaceTasks ? [{
    id: CURRENT_WORKSPACE_ID,
    name: currentConversationTitle?.trim() || 'Chat actual',
    status: 'active',
    conversationCount: 1,
    deliverables: currentConversationTasks.flatMap(task => task.deliverable ? [{
  ...
  const [selectedProjectId, setSelectedProjectId] = useState<string | null>(currentProjectId ?? visibleProjects[0]?.id ?? null)
  ...
  useEffect(() => {
    ...
    if (selectedProjectId === CURRENT_WORKSPACE_ID) {
      setProjectDetail({
        id: CURRENT_WORKSPACE_ID,
        name: currentConversationTitle?.trim() || 'Chat actual',
        status: 'active',
        conversations: currentConversationId ? [{
          ...
          title: currentConversationTitle?.trim() || 'Conversacion actual',
          kanbanTasks: currentConversationTasks,
        }] : [],
      })
      setLoading(false)
      return
  ...
  }, [selectedProjectId, currentConversationId, currentConversationTasks, currentConversationTitle])
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
          {visibleProjects.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-ink-faint">Aun no tienes proyectos ni tareas activas</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-ink-faint">
                        {project.id === CURRENT_WORKSPACE_ID
                          ? `${currentConversationTasks.length} tarea${currentConversationTasks.length === 1 ? '' : 's'} en este chat`
          ...
          <>
            <div className="px-4 md:px-6 py-4 border-b border-edge bg-surface flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    if (projectDetail.id === CURRENT_WORKSPACE_ID) {
                      if (currentConversationId) onOpenConversation(currentConversationId)
                      return
                    }
                    onOpenProject(projectDetail.id)
                  }}
                  className="px-3 py-2 text-xs font-semibold rounded-xl bg-primary text-primary-fg hover:opacity-90 transition-opacity"
                >
                  {projectDetail.id === CURRENT_WORKSPACE_ID ? 'Abrir chat' : 'Abrir proyecto'}
                </button>
              </div>
            </div>
